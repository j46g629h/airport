...
var AUTH = {
  ...
  /* ── 登入效期（v2.8）────────────────────────────────────────
     真正的把關是 IDLE_TTL：**閒置**超過這個時間就自動登出。

     ⚠️ 「閒置」算的是「距離上一次呼叫後端有多久」，不是「登入了多久」。
        只要還在操作，一直用一整天也不會被踢出去；反過來說，
        分頁開著沒關、人離開座位一小時，下一次動作就會要求重新登入。
        工廠的共用電腦上這件事比「登入效期」重要得多。

     ⚠️ TOKEN_TTL 現在**只是垃圾回收的上限**，不是使用者感受得到的規則。
        PropertiesService 沒有自動過期，留一個比 IDLE_TTL 寬的值，
        讓沒人再碰的憑證最後一定會被 storeSweepExpired() 掃掉。
        它必須大於 IDLE_TTL，否則會變成它先到期——那樣使用者會在
        還沒閒置滿一小時的時候就被登出，而且畫面上看不出為什麼。 */
  IDLE_TTL:        1 * 3600,  // 閒置 1 小時自動登出（使用者 2026-09-04 指定）
  TOKEN_TTL:       8 * 3600,  // 垃圾回收上限，一定要 > IDLE_TTL

  /* 每次呼叫 API 都把「最後活動時間」寫回 PropertiesService 的話，
     每一支 API 都要多付一次寫入（約 50~100ms）。隔這麼久才寫一次就夠了——
     ⚠️ 這不會讓閒置時間變得不準：到期與否是拿 session.seen 直接比對算的，
        續期只是「什麼時候把 seen 寫回去」。 */
  IDLE_TOUCH_GAP:  5 * 60,

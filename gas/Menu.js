...
/**
 * 開啟試算表時重算一次航班名冊的重複標記（自我修復）。
 *
 * ── 為什麼需要這一道 ────────────────────────────────────
 *
 * 紅底＋註解**是衍生資料**，跟 `_INDEX` 一樣：真相在航班號那一欄，
 ...
 *
 * ⚠️ 成本可以忽略：航班名冊只有幾十列，一次 getValues ＋ 兩次 set，
 *    大約 0.2~0.4 秒，而且是在開檔案的當下，沒有人在等它。
 *
 * ⚠️ 一定要包 try/catch。onOpen 在「還沒授權」或「唯讀身分開啟」時
 *    寫不進去，那不可以害到選單與跳頁——它們是使用者真的看得到的東西。
 */
function refreshFlightMarks_() {
  try {
    var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEETS.FLIGHT);
    if (sheet) markFlightDuplicates_(sheet);
  } catch (err) {
    Logger.log('重算航班重複標記失敗：' + err.message);
  }
}
